...
icon = Image.open('phn12.jpeg')
st.set_page_config(page_title= "Phonepe Pulse | By Anvitha",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This dashboard app is created by *Anvitha*!
...
with st.sidebar:
    selected = option_menu("Menu",["Home","Top Charts","Explore Data","About"], 
                ...
                default_index=0,
                styles={"nav-link": {"font-size": "20px", "text-align": "left", "margin": "-2px", "--hover-color": "#80542c"},
                        "nav-link-selected": {"background-color": "#c97d36"}})
# MENU 1 - HOME
if selected == "Home":
    #st.image("phn1.jpeg",width=850)
...
# Top Charts - TRANSACTIONS    
    if Type == "Transactions":
        ...
        with col1:
            ...
            mycursor.execute(f"select state, sum(Transaction_count) as Total_Transactions_Count, sum(Transaction_amount) as Total from aggre_trans where year = {Year} and quarter = {Quarter} group by state order by Total desc limit 10")
    ...
    if Type == "Users":
        ...
        with col1:
            ...
            if Year == 2022 and Quarter in [2,3,4]:
                ...
                mycursor.execute(f"select Brands, sum(Count) as Total_Count, avg(Percentage)*100 as Avg_Percentage from aggre_user where year = {Year} and quarter = {Quarter} group by Brands order by Total_Count desc limit 10")
...
# BAR CHART - TOP PAYMENT TYPE
        ...
        mycursor.execute(f"select Transaction_type, sum(Transaction_count) as Total_Transactions, sum(Transaction_amount) as Total_amount from aggre_trans where year= {Year} and quarter = {Quarter} group by Transaction_type order by Transaction_type")
...
if selected == "About":
    ...
    st.markdown("### :orange[About PhonePe Pulse:] ")
    ...
    st.markdown("### :orange[About PhonePe:] ")
